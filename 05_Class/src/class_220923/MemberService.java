...
private MemberDTO[] ar = new MemberDTO[5]; //초기값으로 null
	...
	public void delete() {
		...
		for(i=0; i<ar.length; i++) {
			if(ar[i] != null) { // 방에 입력된 값이 null 값이 아니면 if 문 실행해라
				if(ar[i].getPhone().equals(phone)) {// 방에 입력된 번호 값과 입력한 휴대폰 번호 값 비교
					ar[i] = null; //null 값을 ar[i]에 저장해라
		...
		// for 문을 4까지 돌리고 나올때 5가 되서 ar.length 랑 값이 같아진다.
